...
type StudyMode = 'active-learning' | 'break-mode' | 'focused' | 'review';
...
export async function generateAIResponse(
  ...
  try {
    // Get the Gemini model (gemini-1.5-flash is free and fast)
    const model = genAI.getGenerativeModel({ model: 'gemini-1.5-flash' });

    // Get system prompt based on study mode and language
    const systemPrompt = STUDY_MODE_PROMPTS[studyMode]?.[language] || STUDY_MODE_PROMPTS[studyMode].en;

    // Add language instruction if not English
    const languageInstruction = language !== 'en' 
      ? `\n\nIMPORTANT: You MUST respond ONLY in ${getLanguageName(language)} language. Do not use English.`
      : '';
